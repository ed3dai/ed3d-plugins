# jackal-linear Implementation Plan — Phase 4

**Goal:** Define content standards for Linear-facing text via the `writing-for-linear` skill.

**Architecture:** A new SKILL.md covering three writing contexts: issue descriptions (structured, outcome-focused), status-change comments (brief, links PR/commit, states what changed), and inline comments (conversational, for questions or blockers). The skill is invoked from `linear-workflow` finish mode (Phase 5) to compose comments before posting.
...
<!-- START_TASK_1 -->
### Task 1: Create the writing-for-linear skill

**Files:**
- Create: `plugins/jackal-linear/skills/writing-for-linear/SKILL.md`

**Step 1: Create the skills directory**

```bash
mkdir -p plugins/jackal-linear/skills/writing-for-linear
```

**Step 2: Create SKILL.md**

Create `plugins/jackal-linear/skills/writing-for-linear/SKILL.md` with the following content:
...
Run: `grep -c "Context 1\|Context 2\|Context 3" plugins/jackal-linear/skills/writing-for-linear/SKILL.md`
Expected: `3` (all three writing contexts are present)

**Step 4: Commit**

```bash
git add plugins/jackal-linear/skills/writing-for-linear/SKILL.md
git commit -m "feat(jackal-linear): add writing-for-linear skill"
```
<!-- END_TASK_1 -->
